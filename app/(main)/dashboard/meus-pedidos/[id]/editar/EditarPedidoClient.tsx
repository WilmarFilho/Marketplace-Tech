...
import { updatePurchaseRequest } from "@/app/(main)/explorar/purchase-requests-actions";
...
export default function EditarPedidoClient({ pedido }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        await updatePurchaseRequest(pedido.id, formData);
      ...
      <div className="w-full px-4">
        <div className="mx-auto w-full max-w-2xl px-6 py-10 pb-24 md:px-[40px]">
          ...
          <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-transparent p-6 md:p-8">
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-white mb-1">Editar Pedido</h1>
              <p className="text-gray-400 text-sm">Atualize os detalhes do seu pedido de compra.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  min="0"
                  step="0.01"
                  defaultValue={pedido.target_price ?? ""}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 text-sm focus:outline-none transition-all"
                />
              </div>
              ...
              {/* Telefone */}
              <div className="flex flex-col gap-1.5">
                <label htmlFor="contact_phone" className="text-sm font-medium text-gray-300">Telefone</label>
                <input
                  id="contact_phone"
                  name="contact_phone"
                  type="tel"
                  defaultValue={pedido.contact_phone ?? ""}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 text-sm focus:outline-none transition-all"
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none transition-all appearance-none"
                >
                  <option value="ativo" className="bg-[#1a1a2e]">Ativo — ainda procurando</option>
                  <option value="encerrado" className="bg-[#1a1a2e]">Encerrado — já encontrei</option>
                </select>
              </div>
